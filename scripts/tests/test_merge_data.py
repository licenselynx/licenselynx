...
def test_read_org_data_single_org(tmpdir):
    ...
    assert "siemensMap" in result
    siemens_map = result["siemensMap"]
...
def test_main_integration_with_orgs(monkeypatch):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            assert "siemensMap" in output_data
            ...
            # Verify siemensMap content (canonical + aliases + risky merged)
            assert "SISL-1.1" in output_data["siemensMap"]
            assert "Siemens Inner Source License v1.1" in output_data["siemensMap"]
            assert "sisl-risky" in output_data["siemensMap"]
        finally:
